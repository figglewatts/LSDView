...
namespace LSDView.Util
{
    public class PlyBuilder
    {
        ...
        public PlyBuilder() { _builder = new StringBuilder(); }

        public PlyBuilder WriteHeader(int verts, int faces)
            {
            _builder.AppendLine($"ply\nformat ascii 1.0\n" + // Required header data
                $"comment Generated by LSDView {Version.String}, created by Figglewatts, 2020\n" + // I have to write the comments here, as the first two lines of any PLY file *MUST* be "ply", followed by the format.
        ...
        public PlyBuilder BuildVertexElement(Vector3 pos, Vector3 normal, Vector2 uv, Vector4 col)
        {
            ...
            _builder.AppendLine($"{pos.X} {pos.Y} {pos.Z} {normal.X} {normal.Y} {normal.Z} {uv.X} {uv.Y} {Convert.ToByte(col.X)} {Convert.ToByte(col.Y)} {Convert.ToByte(col.Z)} {Convert.ToByte(col.W)}");
